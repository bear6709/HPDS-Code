...
public class graph3 {
				...
				for( String val : rs){
					...
					if( !sip.equals(dip)){
						...
						if( !temp.toString().equals("[-1,-1,-1]|")){		//have feature edge
							...
							context.write( new Text(line[0]), new Text( fline[0]+temp.toString()));
						}
	...
	public static class IntSumReducer extends Reducer<Text, Text, Text, Text> {
		...
		/*	for( Text val : values){
			...
			StringBuffer temp = new StringBuffer("");
			StringBuffer initscore_temp = new StringBuffer("");
			for( Text val : values){
				if( val.charAt(0) != ':')
					temp.append(val);
				else
					initscore_temp.append(val);
			}
			
			initscore_temp.append(temp);
			
			if( initscore_temp.charAt(initscore_temp.length() - 1) == '|')
				initscore_temp.deleteCharAt( initscore_temp.length() - 1);			
			
			context.write(key, new Text(initscore_temp.toString()));
		}
	}
