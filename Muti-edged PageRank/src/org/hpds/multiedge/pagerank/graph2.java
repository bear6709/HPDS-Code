//pagerank initial score: standard
...
public class graph2 {
	...
	public static class TokenizerMapper extends Mapper<LongWritable, Text, Text, Text> {
		...
		static BigDecimal []mean = {new BigDecimal(0),new BigDecimal(0),new BigDecimal(0)};
		static BigDecimal []standard = {new BigDecimal(0),new BigDecimal(0),new BigDecimal(0)};
		...
		public void map(LongWritable key, Text value, Context context)throws IOException, InterruptedException {
			...
			if( flag == 0){
				...
				feature = new HashMap<String,List<String>>();
				
				BigDecimal zero = new BigDecimal(0);
				BigDecimal [][] sum = {{zero,zero,zero},{zero,zero,zero}};
				long num = 0;
				...
				while( list.ready()){ 
					...
					for( int i = 0; i <= 2; i++){
						sum[0][i] = sum[0][i].add( new BigDecimal(feval[i]));
						sum[1][i] = sum[1][i].add( new BigDecimal(feval[i]).multiply(new BigDecimal(feval[i])));
					}
				}
				
				for( int i = 0; i <= 2; i++){
					mean[i] = sum[0][i].divide( new BigDecimal(num), 5, BigDecimal.ROUND_HALF_UP);
					standard[i] = new BigDecimal( Math.pow((sum[1][i].divide( new BigDecimal(num), 3, BigDecimal.ROUND_HALF_UP)).subtract(mean[i].multiply(mean[i])).doubleValue(), 0.5)).setScale(3, BigDecimal.ROUND_HALF_UP);
				}
			...
			if( value.getLength() > 0){		//0_140.116.164.71        41,16,0.27
				...
				BigDecimal []normalize = {new BigDecimal(0),new BigDecimal(0),new BigDecimal(0)};
				for( int i = 0; i <= 2; i++){
					normalize[i] = ( new BigDecimal(score[i]).subtract(mean[i])).divide(standard[i], 3, BigDecimal.ROUND_HALF_UP);
				}
				String result = ":[" + normalize[0] + "," + normalize[1] + "," + normalize[2] + "]:";
