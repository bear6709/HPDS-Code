				...
				for (int i = 0; i < files.length; i++) {
					if (!files[i].isDir()) {
                        ...
                        while ((line = is.readLine()) != null) {
							...
							for( int j = 0; j <= 2; j++){
								if( new BigDecimal(feval[j]).compareTo(maxfeature[j]) == 1)
									maxfeature[j] = new BigDecimal(feval[j]);
							}
                        }
					}
				}
				...
				for( String val : rs){
					...
					if( !sip.equals(dip)){
						...
						if( !score[0].equals("-1") && !fscore[0].equals("-1")){		//i
							...
							if( a.subtract(b).abs().compareTo(avginterstitial) <= 0){
						...
						if( !score[1].equals("-1") && !fscore[1].equals("-1")){		//r
							...
							if( a.subtract(b).abs().compareTo(ioratio) <= 0){
						...
						if( !score[2].equals("-1") && !fscore[2].equals("-1")){		//t
							...
							if( a.subtract(b).abs().compareTo(packetpersec) <= 0){
								temp.append(a.subtract(b).abs()).append("]|");
								choice++;
							}else
								temp.append("-1]|");
						...
						if( !temp.toString().equals("[-1,-1,-1]|") && choice >= 2)		//have feature edge
